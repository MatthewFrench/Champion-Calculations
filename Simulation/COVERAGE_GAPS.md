# Coverage Gaps Tracker (2026-02-25)
...
- Add runtime loader+script consumption for champion `execution` semantic keys (for example `resolution_timing`, `target_required`, `resets_basic_attack_timer_on_cast`, `empowered_attack_window_seconds`, `max_empowered_attacks`) so attack-cadence-coupled behavior can be consumed without relying only on free-text notes.
- Add explicit loader/runtime semantics for temporary `max_empowered_attacks = 0` sentinel usage (duration-limited variable-hit empowered states) and migrate to a first-class unbounded-within-window schema representation.
...
## Data Quality Gap Snapshot (2026-02-25 Audit, Updated)
...
| Arena teammate-stat-link runtime follow-up | `Twin Mask` now documents page-verified dynamic teammate-stat transfer (live-stat updates + alive/dual-item gated amplification), and raw text/value alignment is normalized to `15%` base / `30%` dual-mask branch | Add runtime ally-state stat-link support so teammate-sharing effects can be simulated when Arena item runtime coverage is expanded |
...
| Gambler's Blade stored-gold cap reconciliation | Current canonical effect text and Tier-1/local tooltip data use `30` to `240` stored-gold range; item now stores patch-history metadata noting temporary V14.12 `245` max | Keep canonical `240` cap as default and preserve `245` as historical context only unless a new current authoritative source supersedes it |
...
| Active-cast execution metadata completeness | Fixed-cooldown active execution metadata queue is now cleared (`22/22` fixed-cooldown `on_activate` effects include explicit `cast_windup_seconds`, `cast_range`, and/or `effect_hitbox_radius`) across touched items including `Hextech Gunblade`, `Mikael's Blessing`, `Mercurial Scimitar`, `Prowler's Claw`, `Quicksilver Sash`, `Redemption`, `Ravenous Hydra`, `Spectral Cutlass`, `Stridebreaker`, `Turbo Chemtank`, and `Youmuu's Ghostblade` | Keep fixed-cooldown active execution-field coverage as a no-regression guardrail and maintain separate tracking for intentional no-fixed-cooldown cadence models |
| No-fixed-cooldown active cadence schema coverage (`effects_structured[].activation_cadence`) | `11/11` currently tracked `on_activate` effects without fixed cooldown now include explicit reusable cadence models (`charge_consumption`, `consumable_single_use`, `single_use_transform`, `round_limited_uses`) across `10` files | Keep schema no-regression and ensure future no-fixed-cooldown activations always include canonical `activation_cadence` metadata |
...
| Rune narrative decimal-spacing normalization (`wiki_descriptions`/touched `long_desc`) | Flat + split rune narrative decimal queue is clear (`0` remaining `x. y` artifacts; `88` cleaned in wave 110) | Keep narrative decimal no-regression audits active alongside flat/split parity checks so reviewer-facing rune text remains mechanically readable |
...
| Champion effect-level context-note truncation audit | `0` effect entries across `0` champion files are currently flagged by strict regex audit as candidate truncated timing fragments in `effects[*].context_notes` (wave 120 repaired previously discovered `XinZhao`/`Fiora` fragments) | Queue is cleared; keep effect-level string+array-aware strict/cadence/article/cast-time sweeps as no-regression guardrails |
...
| Champion execution semantic-key explicitness (`execution.resolution_timing`) | `66/682` active champion abilities now include explicit resolution semantic keys after waves `121-145` (including latest wave additions `DrMundo` `Infected Bonesaw`, `Teemo` `Blinding Dart`, `Jinx` `Zap!`, `Ezreal` `Trueshot Barrage`, `Jax` `Leap Strike`, `Qiyana` `Audacity`, `Vi` `Vault Breaker`, and `Yasuo` `Sweeping Blade`) in addition to earlier baseline coverage | Expand semantic-key coverage on additional attack-cadence-coupled abilities and add deferred runtime loader/script consumption follow-up so these keys are executable |
...
| Champion execution-metadata expressiveness follow-up | Latest waves now document dynamic semantics for `Nasus` (`Spirit Fire` delayed/tick cadence), `Renekton` (`Slice and Dice` move-speed-scaled dash), `Sylas` (`Abscond` two-stage recast flow), `Vex` (`Looming Darkness` cast-distance radius scaling), and expanded execution semantic-key coverage through wave `145` (`66/682` active abilities) including explicit projectile-hit and dash-contact timing classes | Keep canonical notes/source provenance updated, expand semantic-key coverage on attack-cadence/projectile/dash-contact abilities, and track deferred runtime support for formula-speed dashes, stage identity windows, distance-scaled radius interpolation, execution semantic-key consumption, and variable-hit sentinel semantics |
| Variable-hit empowered-attack cap semantics (`max_empowered_attacks = 0`) | Temporary duration-limited variable-hit sentinel usage is currently on `Sivir` (`Ricochet`), `Ashe` (`Ranger's Focus`), `KogMaw` (`Bio-Arcane Barrage`), `Udyr` (`Blazing Stampede`), `MasterYi` (`Wuju Style`), `Twitch` (`Spray and Pray`), and `Xayah` (`Deadly Plumage`) while preserving explicit `resolution_timing` and window semantics | Define first-class explicit schema + loader/runtime handling for unbounded-within-window empowered-hit states and retire temporary sentinel usage after migration |
| Projectile/dash semantic timing runtime follow-up | Champion execution vocabulary now includes `on_projectile_hit` and `on_dash_contact_or_completion`, and touched abilities (`Talon Q`, `Irelia Q`, `Fizz Q`, `Ezreal Q`, `Smolder Q`, `DrMundo Q`, `Teemo Q`, `Jinx W`, `Ezreal R`, `Jax Q`, `Qiyana E`, `Vi Q`, `Yasuo E`) use these values with explicit target/cancel metadata | Add loader/runtime consumption and deterministic event handling for the new timing classes so post-cast contact behavior is simulated without relying only on free-text notes |
...
| Runes Reforged split-structure coverage | `5/5` trees now have split `primary_runes.json` + `secondary_runes.json` files, plus `RunesReforged/RunesReforged.json` index and `StatShards/stat_shards.json`; latest parity audit confirms `61/61` rune IDs and `3/3` stat shard slots aligned with flat file | Keep split tree structure as authoring baseline while maintaining flat-file compatibility (`Masteries/RunesReforged.json`) until runtime loader migration |

## Data Coverage Progress (2026-02-25)
...
- Completed:
  ...
  - completed champion execution-semantics wave 107 (`Olaf` `Reckless Swing`):
    - encoded explicit execution semantic keys for target-required cast completion and attack-speed-scaled windup cancellation behavior
    - refreshed page-level provenance and schema context notes for future runtime-consumption follow-up planning
  - completed champion execution-semantics wave 108 (`Fiora` `Bladework`):
    - encoded explicit empowered-basic-attack sequence metadata (`resolution_timing`, attack-reset arming, two-hit window/count semantics)
    - added page-level `Bladework` provenance to champion sources and tracker wave metadata
  - completed item active-cooldown completeness wave 109 (`Stridebreaker`, `Hextech Rocketbelt`, `Redemption`):
    - backfilled source-verified active cooldown metadata across all active structured-effect branches (`15s`, `40s`, `90s` respectively)
    - refreshed per-item context notes/sources to document cooldown provenance and branch-level consistency
  - completed rune narrative decimal-normalization wave 110:
    - normalized rune narrative decimal-spacing artifacts across flat/split files (`88` `x. y` artifacts -> `0`)
    - preserved flat/split parity while keeping existing structured rune numeric metadata stable
  - completed champion execution-semantics wave 111 (`Nasus` `Siphoning Strike`, `Garen` `Decisive Strike`):
    - encoded explicit attack-cadence semantic keys (`resolution_timing`, `target_required`, `resets_basic_attack_timer_on_cast`, `max_empowered_attacks`, `windup_scales_with_bonus_attack_speed`, `cancels_if_target_invalid_before_resolution`) for cast-prime plus empowered-hit behavior
    - added page-level ability template provenance and tracker wave scope for both champions
  - completed champion execution-semantics wave 112 (`Jax` `Empower`, `Renekton` `Ruthless Predator`):
    - encoded explicit attack-cadence semantic keys for empowered-hit resolution and attack-reset cadence behavior (including `Jax` leap-strike consume path semantics)
    - added page-level ability template provenance and tracker wave scope for both champions
  - completed champion execution-semantics wave 113 (`MonkeyKing` `Crushing Blow`, `Blitzcrank` `Power Fist`):
    - encoded explicit attack-cadence semantic keys for empowered-hit control/damage resolution and uncancellable-windup behavior
    - added page-level ability template provenance and tracker wave scope for both champions
  - completed champion execution-semantics wave 114 (`Leona` `Shield of Daybreak`, `Vayne` `Tumble`):
    - encoded explicit attack-cadence semantic keys, including two-phase dash-then-empowered-hit modeling for `Tumble`
    - added page-level ability template provenance and tracker wave scope for both champions
  - completed item active-cooldown completeness wave 115 (`Ravenous Hydra`):
    - backfilled page-verified `10s` active cooldown metadata on `ravenous_crescent_active_physical_damage`
    - updated context/provenance notes and re-reviewed structured item metadata (`effects_structured_reviewed = 2026-02-24`)
  - completed champion execution-semantics wave 116 (`Camille` `Precision Protocol`, `Ekko` `Phase Dive`):
    - encoded explicit `resolution_timing` and companion attack-cadence semantic keys for cast-prime versus empowered-hit behavior
    - recorded page-level champion ability citations in tracker wave scope for both champions
  - completed champion execution-semantics wave 117 (`Rengar` `Savagery`, `XinZhao` `Three Talon Strike`):
    ...
    - added `Three Talon Strike` page-level source provenance and tracker wave scope for both champions
  ...
  - completed champion effect-level truncation normalization wave 120 (`XinZhao` `Wind Becomes Lightning`, `Fiora` `Riposte`):
    - repaired discovered effect-level timing-note fragments (`5` entries -> `0`) and aligned effect-note timing semantics to page-level template text
    - added page-level champion ability citations in tracker wave scope and champion-file `sources` for both touched champions
    - expanded no-regression truncation auditing scope to include `effects[*].context_notes` alongside ability-level notes
  - completed item activation-cadence schema wave 120 (item pass):
    - backfilled reusable `activation_cadence` metadata for all currently tracked no-fixed-cooldown `on_activate` effects (`11/11` across `10` files)
    - normalized cadence-model classification (`charge_consumption`, `consumable_single_use`, `single_use_transform`, `round_limited_uses`) and updated per-item context notes
  - completed champion execution-semantics waves 121-124 (`Darius` `Crippling Strike`, `Shyvana` `Twin Bite`, `RekSai` `Queen's Wrath`, `Chogath` `Vorpal Spikes`, `Skarner` `Shattered Earth`, `Illaoi` `Harsh Lesson`, `Draven` `Spinning Axe`, `Gwen` `Skip 'n Slash`):
    - encoded explicit attack-cadence semantic keys for cast-prime plus empowered-hit timing and two-phase cast-state resolution where source-verified
    - added page-level ability-template provenance and recorded tracker scope in `manual_behavior_verification_wave_121` through `manual_behavior_verification_wave_124`
  - completed champion execution-semantics waves 125-128 (`Fizz` `Seastone Trident`, `Malphite` `Thunderclap`, `Shen` `Twilight Assault`, `Gragas` `Drunken Rage`, `Hecarim` `Devastating Charge`, `Kassadin` `Nether Blade`, `Kayle` `Starfire Spellblade`, `Viktor` `Siphon Power`):
    - expanded explicit `resolution_timing` baseline to `34/682` active abilities with canonical vocabulary and companion attack-cadence semantic keys
    - added page-level ability-template provenance and recorded tracker scope in `manual_behavior_verification_wave_125` through `manual_behavior_verification_wave_128`
  - completed champion execution-semantics waves 130-133 (`Sett` `Knuckle Down`, `Volibear` `Thundering Smash`, `TwistedFate` `Pick a Card`, `Shaco` `Deceive`, `Zac` `Stretching Strikes`, `Udyr` `Wilding Claw`, `Udyr` `Iron Mantle`, `Sivir` `Ricochet`):
    - expanded explicit `resolution_timing` baseline to `42/682` active abilities with canonical vocabulary and companion attack-cadence semantic keys
    - added page-level ability-template provenance and recorded tracker scope in `manual_behavior_verification_wave_130` through `manual_behavior_verification_wave_133`
    - documented temporary duration-limited variable-hit sentinel fallback (`max_empowered_attacks = 0`) on `Sivir` `Ricochet` and tracked explicit-schema/runtime follow-up
  - completed champion execution-semantics waves 134-141 (`Ashe` `Ranger's Focus`, `KogMaw` `Bio-Arcane Barrage`, `Udyr` `Blazing Stampede`, `MasterYi` `Wuju Style`, `Twitch` `Spray and Pray`, `Xayah` `Deadly Plumage`, `Rammus` `Defensive Ball Curl`, `Olaf` `Tough It Out`, `DrMundo` `Blunt Force Trauma`, `Zaahen` `The Darkin Glaive`, `Jayce` `Transform Mercury Cannon`, `Talon` `Noxian Diplomacy`, `Irelia` `Bladesurge`, `Fizz` `Urchin Strike`, `Ezreal` `Mystic Shot`, `Smolder` `Super Scorcher Breath`):
    - expanded explicit `resolution_timing` baseline to `58/682` active abilities with canonical vocabulary and companion execution semantic keys
    - expanded canonical `execution.resolution_timing` vocabulary with `on_projectile_hit` and `on_dash_contact_or_completion` and applied both values to touched abilities
    - added page-level ability-template provenance and recorded tracker scope in `manual_behavior_verification_wave_134` through `manual_behavior_verification_wave_141`
    - expanded temporary duration-limited variable-hit sentinel queue (`max_empowered_attacks = 0`) to the current tracked set (`Sivir`, `Ashe`, `KogMaw`, `Udyr` `Blazing Stampede`, `MasterYi`, `Twitch`, `Xayah`)
    - corrected discovered truncation defect on `Smolder` `Super Scorcher Breath` tier-2 bolt context note (`Tier 2 - 125 stacks: The fireball sends forth 2 (+ 0.` -> full source-aligned bolt scaling and explosion semantics)
  - completed champion execution-semantics waves 142-145 (`DrMundo` `Infected Bonesaw`, `Teemo` `Blinding Dart`, `Jinx` `Zap!`, `Ezreal` `Trueshot Barrage`, `Jax` `Leap Strike`, `Qiyana` `Audacity`, `Vi` `Vault Breaker`, `Yasuo` `Sweeping Blade`):
    - expanded explicit `resolution_timing` baseline to `66/682` active abilities with canonical vocabulary and companion execution semantic keys
    - added page-level ability-template provenance and recorded tracker scope in `manual_behavior_verification_wave_142` through `manual_behavior_verification_wave_145`
    - documented remaining target-invalid pre-resolution edge uncertainty for `Teemo` `Blinding Dart`, `Jax` `Leap Strike`, `Qiyana` `Audacity`, and `Yasuo` `Sweeping Blade` for deferred runtime-policy follow-up
  - completed item active execution-field completeness wave 129:
    - backfilled fixed-cooldown `on_activate` execution fields to full branch coverage (`22/22`) with explicit `cast_windup_seconds`, `cast_range`, and/or `effect_hitbox_radius`
    - maintained no-fixed-cooldown activation-cadence coverage at `11/11` effects across `10` files
  ...
  - completed champion provenance-hardening wave 80:
    ...
    - normalized a discovered truncation defect in `Poppy` `Keeper's Verdict` effect context notes (`charged for at least 0.` -> full threshold semantics)
    - added page-level champion ability citations for each touched ability and recorded wave scope in `Simulation/champion_behavior_verification_tracker.json`
  - completed item confidence-reconciliation wave 82:
    - extended `Dragonheart` immediate backfill branch with inferred acquisition-round thresholds (`3-4 => 1`, `5-6 => 2`, `7-8 => 3`, `9+ => 4`) grounded in published two-round cadence and cap constraints, and raised branch confidence above low-confidence floor
    - reconciled `Gambler's Blade` historical `245` cap note as patch-history context while retaining current canonical `240` cap in structured behavior
  - completed data-quality audit wave 83:
    - re-ran split-vs-flat rune parity checks and confirmed no drift (`61/61` rune IDs aligned, `3/3` shard slots aligned)
    - re-ran champion no-regression audits (`description_source` completeness, context-note truncation queues, and tracker integrity) and confirmed clean state after waves 80-82
  - completed data-quality wave 79 (items + runes parity):
    - encoded `Dragonheart` acquisition-round immediate soul backfill as structured data (`1-4` soul range, cap `4`) and refreshed notes/provenance for model intent
    - normalized `Twin Mask` raw text to match page-verified structured transfer values (`15%` base / `30%` dual-mask branch)
    - ran full split-vs-flat runes parity audit and confirmed no drift (`61/61` rune IDs aligned, `3/3` shard slots aligned)
  - completed champion confidence wave 78:
    - re-reviewed `Zyra` passive (`Garden of Thorns`) one-vs-two seed behavior using template + patch-history sources
    - confirmed stochastic behavior is documented but deterministic weighting remains unpublished; recorded confidence follow-up scope in tracker/confidence docs
  - completed champion provenance-hardening wave 77:
    - re-verified and hardened source-corpus `description_source` phrasing for `Aurelion Sol` (`Q`), `Varus` (`Q`), `Jhin` (`R`), and `Thresh` (`Q`)
    - added/used page-level champion ability citations for each touched ability and recorded wave scope in `Simulation/champion_behavior_verification_tracker.json`
  - completed champion provenance-hardening wave 76:
    - re-verified and hardened source-corpus `description_source` phrasing for `Aatrox` (`Q`), `Blitzcrank` (`Q`), `Caitlyn` (`R`), and `Darius` (`E`)
    - added page-level champion ability citations for each touched ability and recorded wave scope in `Simulation/champion_behavior_verification_tracker.json`
...
34. Expand champion `execution` semantic-key coverage (`resolution_timing` and related attack-cadence keys) beyond current `66/682` baseline, prioritizing high-impact empowered-hit and cast-gated melee abilities.
35. Complete deferred runtime follow-up to consume champion execution semantic keys from data loaders/scripts instead of relying only on free-text `context_notes`.
36. Maintain fixed-cooldown active execution-field completeness at `22/22` and preserve no-fixed-cooldown cadence-schema tracking (`10` files / `11` effects) as separate no-regression guardrails.
37. Keep intentional no-fixed-cooldown `on_activate` semantics (charge/consume/single-use/round-limited) explicitly documented in structured metadata and notes (`activation_cadence`, `conditions`, `schema_notes.context_notes`) so they are not conflated with missing cooldown defects.
38. Maintain canonical enum-like vocabulary governance for `execution.resolution_timing` and `activation_cadence.model` through `Simulation/data/execution_semantics_vocabulary.json` and update docs + data together when extending that vocabulary.
39. Maintain effect-level truncation no-regression at `0/0` on `effects[*].context_notes` with the same strict/cadence/article/cast-time sweeps used for ability-level notes.
40. Replace temporary duration-limited variable-hit sentinel usage (`max_empowered_attacks = 0`) with explicit schema + loader/runtime semantics for unbounded-within-window empowered-hit behavior.
41. Resolve remaining target-invalid pre-resolution ambiguity for recent dash/projectile timing updates (`Irelia` `Bladesurge`, `Fizz` `Urchin Strike`, `Smolder` `Super Scorcher Breath`, `Teemo` `Blinding Dart`, `Jax` `Leap Strike`, `Qiyana` `Audacity`, `Yasuo` `Sweeping Blade`) with deeper edge-case verification and corresponding runtime handling policy.
...
10. Rune Narrative Decimal-Hygiene Plan:
   - phase 1 (completed): normalize flat/split rune narrative decimal-spacing artifacts generated by source extraction (`88` `x. y` artifacts -> `0`).
   - phase 2 (in progress): keep no-regression audits active for rune narrative fields (`wiki_descriptions` and touched `long_desc`) while preserving flat/split parity.
   - phase 3 (pending code follow-up): add optional loader/lint audit for narrative decimal artifacts so generated spacing regressions are caught automatically.
...
- Execution semantics vocabulary:
  - `Simulation/data/execution_semantics_vocabulary.json`
