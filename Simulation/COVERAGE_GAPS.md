...
| Active-cast execution metadata completeness | Targeted active metadata normalization now covers `Mikael's Blessing`, `Hextech Gunblade`, `Mercurial Scimitar`, `Profane Hydra`, `Randuin's Omen`, `Shurelya's Battlesong`, `Youmuu's Ghostblade`, `Wordless Promise`, `Stridebreaker`, `Hextech Rocketbelt`, `Redemption`, and `Ravenous Hydra` with explicit cooldown/cast-range-or-radius semantics where source text provides values | Continue broader audit with queue stratification: fixed-cooldown actives are prioritized for cooldown backfill; intentional charge/consume/single-use actives are tracked separately (`10` `on_activate` files currently have no fixed `cooldown_seconds`) |
...
| Champion execution semantic-key explicitness (`execution.resolution_timing`) | `18/682` active champion abilities now include explicit resolution semantic keys (`Olaf` `Reckless Swing`, `Fiora` `Bladework`, `Nasus` `Siphoning Strike`, `Garen` `Decisive Strike`, `Jax` `Empower`, `Renekton` `Ruthless Predator`, `MonkeyKing` `Crushing Blow`, `Blitzcrank` `Power Fist`, `Leona` `Shield of Daybreak`, `Vayne` `Tumble`, `Camille` `Precision Protocol`, `Ekko` `Phase Dive`, `Rengar` `Savagery`, `XinZhao` `Three Talon Strike`, `Yorick` `Last Rites`, `Trundle` `Chomp`, `Volibear` `Frenzied Maul`, `Vi` `Relentless Force`) in addition to baseline windup/projectile/radius fields | Expand semantic-key coverage on additional attack-cadence-coupled abilities and add deferred runtime loader/script consumption follow-up so these keys are executable |
...
| Champion execution-metadata expressiveness follow-up | Latest waves now document dynamic semantics for `Nasus` (`Spirit Fire` delayed/tick cadence), `Renekton` (`Slice and Dice` move-speed-scaled dash), `Sylas` (`Abscond` two-stage recast flow), `Vex` (`Looming Darkness` cast-distance radius scaling), plus expanded explicit attack-cadence semantic-key coverage on `Olaf` `Reckless Swing`, `Fiora` `Bladework`, `Nasus` `Siphoning Strike`, `Garen` `Decisive Strike`, `Jax` `Empower`, `Renekton` `Ruthless Predator`, `MonkeyKing` `Crushing Blow`, `Blitzcrank` `Power Fist`, `Leona` `Shield of Daybreak`, `Vayne` `Tumble`, `Camille` `Precision Protocol`, `Ekko` `Phase Dive`, `Rengar` `Savagery`, `XinZhao` `Three Talon Strike`, `Yorick` `Last Rites`, `Trundle` `Chomp`, `Volibear` `Frenzied Maul`, and `Vi` `Relentless Force` | Keep canonical notes/source provenance updated, expand semantic-key coverage on attack-cadence abilities, and track deferred runtime support for formula-speed dashes, stage identity windows, distance-scaled radius interpolation, and execution semantic-key consumption |
...
34. Expand champion `execution` semantic-key coverage (`resolution_timing` and related attack-cadence keys) beyond current `18/682` baseline, prioritizing high-impact empowered-hit and cast-gated melee abilities.
...
36. Maintain active-item cooldown metadata completeness progress (current `on_activate` files without fixed `cooldown_seconds`: `10` files / `11` effects) and continue source-verified branch-level cooldown backfills for fixed-cooldown actives.
