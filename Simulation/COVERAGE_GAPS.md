...
| Active-cast execution metadata completeness | Targeted active metadata normalization now covers `Mikael's Blessing`, `Hextech Gunblade`, `Mercurial Scimitar`, `Profane Hydra`, `Randuin's Omen`, `Shurelya's Battlesong`, `Youmuu's Ghostblade`, `Wordless Promise`, `Stridebreaker`, `Hextech Rocketbelt`, `Redemption`, and `Ravenous Hydra` with explicit cooldown/cast-range-or-radius semantics where source text provides values | Continue broader audit with queue stratification: fixed-cooldown actives are prioritized for cooldown backfill, while intentional charge/consume/single-use actives remain a separate queue (`10` `on_activate` files currently have no fixed `cooldown_seconds`) |
...
| Champion execution semantic-key explicitness (`execution.resolution_timing`) | `10/682` active champion abilities now include explicit resolution semantic keys (`Olaf` `Reckless Swing`, `Fiora` `Bladework`, `Nasus` `Siphoning Strike`, `Garen` `Decisive Strike`, `Jax` `Empower`, `Renekton` `Ruthless Predator`, `MonkeyKing` `Crushing Blow`, `Blitzcrank` `Power Fist`, `Leona` `Shield of Daybreak`, `Vayne` `Tumble`) in addition to baseline windup/projectile/radius fields | Expand semantic-key coverage on additional attack-cadence-coupled abilities and add deferred runtime loader/script consumption follow-up so these keys are executable |
...
34. Expand champion `execution` semantic-key coverage (`resolution_timing` and related attack-cadence keys) beyond current `10/682` baseline, prioritizing high-impact empowered-hit and cast-gated melee abilities.
...
36. Maintain active-item cooldown metadata completeness progress (current `on_activate` files without fixed `cooldown_seconds`: `10`) and continue source-verified branch-level cooldown backfills for fixed-cooldown actives.
37. Keep intentional no-fixed-cooldown `on_activate` semantics (charge/consume/single-use/round-limited) explicitly documented in structured `conditions` and `schema_notes.context_notes` so they are not conflated with missing cooldown defects.
