...
| Champion execution semantic-key explicitness (`execution.resolution_timing`) | `42/682` active champion abilities now include explicit resolution semantic keys after waves `121-133` (including new wave additions `Sett`, `Volibear`, `TwistedFate`, `Shaco`, `Zac`, `Udyr`, and `Sivir`) in addition to earlier baseline coverage | Expand semantic-key coverage on additional attack-cadence-coupled abilities and add deferred runtime loader/script consumption follow-up so these keys are executable |
...
| Champion execution-metadata expressiveness follow-up | Latest waves now document dynamic semantics for `Nasus` (`Spirit Fire` delayed/tick cadence), `Renekton` (`Slice and Dice` move-speed-scaled dash), `Sylas` (`Abscond` two-stage recast flow), `Vex` (`Looming Darkness` cast-distance radius scaling), and expanded attack-cadence semantic-key coverage through wave `133` (`42/682` active abilities) | Keep canonical notes/source provenance updated, expand semantic-key coverage on attack-cadence abilities, and track deferred runtime support for formula-speed dashes, stage identity windows, distance-scaled radius interpolation, execution semantic-key consumption, and variable-hit sentinel semantics |
| Variable-hit empowered-attack cap semantics (`max_empowered_attacks = 0`) | `Sivir` `Ricochet` now uses temporary `max_empowered_attacks = 0` sentinel to represent duration-limited variable-hit behavior while preserving explicit `resolution_timing` and window semantics | Define first-class explicit schema + loader/runtime handling for unbounded-within-window empowered-hit states and retire temporary sentinel usage after migration |
...
34. Expand champion `execution` semantic-key coverage (`resolution_timing` and related attack-cadence keys) beyond current `42/682` baseline, prioritizing high-impact empowered-hit and cast-gated melee abilities.
