...
| Active-cast execution metadata completeness | Targeted active metadata normalization now covers `Mikael's Blessing`, `Hextech Gunblade`, `Mercurial Scimitar`, `Profane Hydra`, `Randuin's Omen`, `Shurelya's Battlesong`, `Youmuu's Ghostblade`, `Wordless Promise`, `Stridebreaker`, `Hextech Rocketbelt`, and `Redemption` with explicit cooldown/cast-range-or-radius semantics where source text provides values | Continue broader audit so targeted item actives consistently encode cooldown and cast-range metadata when sources publish them (`18` active-item files still have no explicit structured cooldown metadata) |
...
| Champion execution semantic-key explicitness (`execution.resolution_timing`) | `2/682` active champion abilities now include explicit resolution semantic keys (`Olaf` `Reckless Swing`, `Fiora` `Bladework`) in addition to baseline windup/projectile/radius fields | Expand semantic-key coverage on additional attack-cadence-coupled abilities and add deferred runtime loader/script consumption follow-up so these keys are executable |
...
| Champion execution-metadata expressiveness follow-up | Latest waves now document dynamic semantics for `Nasus` (`Spirit Fire` delayed/tick cadence), `Renekton` (`Slice and Dice` move-speed-scaled dash), `Sylas` (`Abscond` two-stage recast flow), `Vex` (`Looming Darkness` cast-distance radius scaling), `Olaf` (`Reckless Swing` target-required cast-complete resolution), and `Fiora` (`Bladework` two-hit empowered attack sequence) | Keep canonical notes/source provenance updated, expand semantic-key coverage on attack-cadence abilities, and track deferred runtime support for formula-speed dashes, stage identity windows, distance-scaled radius interpolation, and execution semantic-key consumption |
...
34. Expand champion `execution` semantic-key coverage (`resolution_timing` and related attack-cadence keys) beyond current `2/682` baseline, prioritizing high-impact empowered-hit and cast-gated melee abilities.
...
36. Maintain active-item cooldown metadata completeness progress (current no-structured-cooldown queue: `18` active-item files) and continue source-verified branch-level cooldown backfills.
