...
pub(crate) fn apply_behavior(profile: &mut ChampionBehaviorProfile) {
    if let Some(silver_bolts) = vayne_silver_bolts_ability_defaults(CHAMPION_KEY) {
...
pub(crate) fn event_cooldown_seconds(event: ChampionScriptEvent) -> Option<f64> {
    ...
    vayne_tumble_ability_defaults(CHAMPION_KEY).map(|defaults| defaults.tumble_cooldown_seconds)
}

pub(crate) fn execute_tumble_empower(
    input: ChampionScriptExecutionInput,
) -> Vec<ChampionScriptAction> {
    let Some(ability_defaults) = vayne_tumble_ability_defaults(CHAMPION_KEY) else {
        return Vec::new();
    };
