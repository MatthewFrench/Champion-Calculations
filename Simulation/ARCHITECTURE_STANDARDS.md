...
- Shared root-level simulator contracts (runtime/search/reporting/CLI option types) should live under `simulation_contracts/*`, not as inline type blocks in `main.rs`.
...
| Trace lifecycle, status projection, and state-snapshot reporting | `engine/trace_snapshot_reporting.rs` facade plus explicit leaves under `engine/trace_snapshot_reporting/*` (integrated by `engine.rs`) | `engine/trace_snapshot_reporting.rs` facade + explicit leaves (`trace_lifecycle_channels.rs`, `trace_status_and_projectile_projections.rs`, `trace_snapshot_summary_projection.rs`) | `enable_trace`, `trace_events`, `trace_event`, `emit_trace_snapshots_due`, `collect_state_snapshot_summary` owner APIs |
| Enemy runtime state collection | `engine/actor_state/enemy_runtime_state.rs` facade plus explicit leaves under `engine/actor_state/enemy_runtime_state/*` (integrated by `engine.rs`) | `engine/actor_state/enemy_runtime_state.rs` facade + explicit leaves (`enemy_trace_snapshot_projections.rs`, `enemy_attack_and_script_channels.rs`, `enemy_lifecycle_channels.rs`) | `enemy_*` runtime owner APIs |
...
| Enemy runtime respawn/regeneration lifecycle | `engine/actor_state/enemy_runtime_state.rs` facade plus explicit lifecycle leaf `enemy_lifecycle_channels.rs` (integrated by `engine.rs`) | `engine/actor_state/enemy_runtime_state.rs` facade + `engine/actor_state/enemy_runtime_state/enemy_lifecycle_channels.rs` | `apply_enemy_respawn_updates`, `apply_enemy_regeneration_tick` |
| Enemy script lifecycle, cadence readiness, and script-runtime mutation | `engine/actor_state/enemy_runtime_state.rs` facade plus explicit leaves (`enemy_attack_and_script_channels.rs`, `enemy_lifecycle_channels.rs`) (integrated by `engine.rs`) | `engine/actor_state/enemy_runtime_state.rs` facade + explicit script/attack leaves | `enemy_script_event_should_recur`, `enemy_script_epoch_matches`, `enemy_script_event_ready_at_or_zero`, `set_enemy_script_event_ready_at`, `enemy_ability_haste_or_urf_default`, `execute_enemy_script_event_actions`, `enemy_aftershock_magic_damage_on_immobilize` |
| Enemy auto-attack token lifecycle and next-hit bonus consume/reset | `engine/actor_state/enemy_runtime_state.rs` facade plus explicit attack leaf `enemy_attack_and_script_channels.rs` (integrated by `engine.rs`) | `engine/actor_state/enemy_runtime_state.rs` facade + `engine/actor_state/enemy_runtime_state/enemy_attack_and_script_channels.rs` | `begin_enemy_attack_sequence`, `enemy_attack_sequence_matches`, `consume_enemy_attack_damage_with_on_hit`, `apply_enemy_next_attack_bonus_physical` |
| Enemy runtime read projections and trace-snapshot composition | `engine/actor_state/enemy_runtime_state.rs` facade plus explicit projection leaf `enemy_trace_snapshot_projections.rs` (integrated by `engine.rs`) | `engine/actor_state/enemy_runtime_state.rs` facade + `engine/actor_state/enemy_runtime_state/enemy_trace_snapshot_projections.rs` | `enemy_name`, `enemy_position`, `enemy_hitbox_radius`, `enemy_attack_range`, `enemy_attack_windup_seconds`, `enemy_attack_projectile_speed`, `enemy_attack_effect_hitbox_radius`, `enemy_target_health_snapshot_or_defaults`, `enemy_attack_interval_seconds`, `enemy_status_lines_at`, `enemy_is_stunned_at`, `enemy_is_invulnerable_or_untargetable_at`, `enemy_count`, `enemy_trace_snapshot_at` |
...
| Runtime loadout stack/cooldown mutation, combat bonus resolution, rune-proc telemetry accumulation, runtime-state initialization/reset, and runtime read-only projections | `scripts/runtime/loadout_runtime.rs` facade, explicit schema/projection/mutation leaves under `scripts/runtime/loadout_runtime/*` (`runtime_state_schema.rs`, `rune_proc_telemetry.rs`, `runtime_state_initialization.rs`, `runtime_effect_mutations.rs`, `runtime_stat_projections.rs`, `runtime_state_reporting.rs`), combat bonus facade + explicit leaves under `scripts/runtime/loadout_runtime/combat_bonus_resolution/*`, and `engine.rs` call sites | `scripts/runtime/loadout_runtime.rs` facade + runtime owner leaves (`runtime_state_schema.rs`, `combat_bonus_resolution.rs`, `combat_bonus_resolution/*`, `rune_proc_telemetry.rs`, `runtime_state_initialization.rs`, `runtime_effect_mutations.rs`, `runtime_stat_projections.rs`, `runtime_state_reporting.rs`) | Script/runtime owner APIs only (`calculate_*`, `on_*`, `trigger_*`, `loadout_*`, `reset_*`, `build_*`, `rune_proc_telemetry`, `describe_runtime_*`) |
...
| Full-loadout search orchestration (strategy dispatch, seed-ensemble aggregation, adaptive/bleed generation) | `search/full_loadout_search_orchestration.rs` facade + explicit owner leaves under `search/full_loadout_search_orchestration/*` (integrated by `search.rs`) | `search/full_loadout_search_orchestration.rs` facade + explicit leaves (`strategy_dispatch.rs`, `seed_elite_generation.rs`, `adaptive_candidate_generation.rs`, `bleed_candidate_generation.rs`) | `build_search_ranked_full_loadout`, `strategy_seed_elites_full_loadout`, `adaptive_strategy_candidates_full_loadout`, `generate_bleed_candidates_full_loadout` |
| Search strategy execution | `search/strategy/item_candidate_search_strategies.rs` facade + explicit leaves under `search/strategy/item_candidate_search_strategies/*`, `search/strategy/full_loadout_search_strategies.rs` facade + explicit leaves under `search/strategy/full_loadout_search_strategies/*`, and `search.rs` facade dispatch | `search/strategy/*` | `*_search_ranked`, strategy rollout/selection owner APIs (`available_actions`, `rollout_completion`) |
...
| Controlled champion scenario runtime/search support helpers (coverage-asset locking, partial-candidate completion, progress/trace telemetry helpers) | `scenario_runner/controlled_champion_search_runtime_support.rs` plus explicit leaves under `scenario_runner/controlled_champion_search_runtime_support/*` (integrated by `scenario_runner.rs`) | `scenario_runner/controlled_champion_search_runtime_support.rs` facade + explicit owner leaves (`coverage_locked_asset_candidate_generation.rs`, `search_seed_derivation.rs`, `search_runtime_reporting_projections.rs`) | `coverage_locked_assets`, `random_locked_candidate`, `mutate_locked_candidate`, `partial_candidate_completion_seed`, `complete_partial_candidate_to_full`, `structured_trace_event`, `append_rune_proc_telemetry_markdown_entries`, `rune_proc_telemetry_json` owner APIs |
| Controlled champion candidate-search phase orchestration (maximum-quality coverage stage, ensemble-seed strategy orchestration, candidate merge/dedupe, strict full ranking) | `scenario_runner/controlled_champion_candidate_search.rs` facade + explicit leaves (`coverage_stage_execution.rs`, `seed_and_strict_execution.rs` facade + `seed_and_strict_execution/*`) (integrated by `scenario_runner/controlled_champion_scenario_runner.rs`) | `scenario_runner/controlled_champion_candidate_search.rs` facade + explicit leaves (`coverage_stage_execution.rs`, `seed_and_strict_execution.rs`, `seed_and_strict_execution/seed_candidate_collection.rs`, `seed_and_strict_execution/strict_candidate_scoring.rs`) | `run_maximum_quality_coverage_stage`, `run_seed_and_strict_ranking` owner APIs |
| Scenario fixed-loadout and rune-sweep execution entrypoints | `scenario_runner/fixed_loadout_runner.rs`, `scenario_runner/rune_sweep_runner.rs`, shared enemy-scenario projection owner `scenario_runner/controlled_champion_enemy_scenario_projection.rs`, and explicit rune-sweep leaves under `scenario_runner/rune_sweep_runner/*` (integrated by `scenario_runner.rs`) | `scenario_runner/fixed_loadout_runner.rs` + `scenario_runner/rune_sweep_runner.rs` facades + shared enemy-scenario projection owner `controlled_champion_enemy_scenario_projection.rs` + explicit rune-sweep owner leaves (`result_aggregation.rs`, `report_writing.rs`) | fixed-loadout/rune-sweep execution owner APIs plus shared projection owner APIs (`parse_scaled_enemy_scenarios`, `build_enemy_build_projection`, `build_scenario_reference_outcomes`) and rune-sweep owner APIs (`average_*`, `write_rune_sweep_reports`) |
| Scenario controlled-champion execution entrypoint | `scenario_runner/controlled_champion_scenario_runner.rs` facade plus execution owner leaf/sub-leaves under `scenario_runner/controlled_champion_scenario_runner/controlled_champion_scenario_execution*` (integrated by `scenario_runner.rs`) | `scenario_runner/controlled_champion_scenario_runner.rs` facade + execution owner leaf `controlled_champion_scenario_execution.rs` + explicit sub-leaves (`deadline_and_progress.rs`, `runtime_setup.rs`, `search_execution.rs`, `search_execution/candidate_scoring_channels.rs`, `projection_dispatch.rs`) | `run_controlled_champion_scenario_impl` owner API |
...
| Controlled champion ranked-build analysis, diagnostics assembly, and build-order analysis | `scenario_runner/controlled_champion_result_build_analysis.rs` facade + explicit sub-leaves under `scenario_runner/controlled_champion_result_build_analysis/*` (integrated by `scenario_runner/controlled_champion_result_reporting.rs`) | `scenario_runner/controlled_champion_result_build_analysis.rs` facade + explicit owner leaves (`candidate_metrics_projection.rs`, `search_diagnostics_projection.rs`, `build_order_analysis.rs`) | `analyze_controlled_champion_build_results` owner API |
...
| Champion simulation-default loading from champion canonical data | `defaults/champion_item_simulation_defaults_loader/champion_simulation_defaults_loaders.rs` plus explicit champion leaves under `defaults/champion_item_simulation_defaults_loader/champion_simulation_defaults_loaders/*` (integrated by `defaults/champion_item_simulation_defaults_loader.rs`) | `defaults/champion_item_simulation_defaults_loader/champion_simulation_defaults_loaders.rs` facade + champion leaves (`vladimir_simulation_defaults_loader.rs`, `warwick_simulation_defaults_loader.rs`, `vayne_simulation_defaults_loader.rs`, `morgana_simulation_defaults_loader.rs`, `sona_simulation_defaults_loader.rs`, `doctor_mundo_simulation_defaults_loader.rs`) | champion simulation-default loader owner APIs (`load_*_defaults`) |
...
| Champion simulation metadata, slot-binding derivation, champion AI profile normalization, and URF respawn-default loading | `defaults/champion_simulation_data_loading.rs` (integrated by `defaults.rs`) | `defaults/champion_simulation_data_loading.rs` | metadata/slot/profile loader owner APIs (`load_champion_simulation_data`, `load_champion_slot_bindings`, `load_champion_ai_profiles`, `load_urf_respawn_defaults`) |
...
| Simulator/default schema-type ownership | `defaults/simulator_defaults_schema_types.rs` plus explicit schema leaves under `defaults/simulator_defaults_schema_types/*` (integrated by `defaults.rs`) | `defaults/simulator_defaults_schema_types.rs` facade + explicit schema owner leaves (`simulation_search_and_engine_defaults_schema.rs`, `rune_runtime_defaults_schema.rs`, `champion_ai_and_execution_schema.rs`, `champion_file_defaults_schema.rs`) + behavior/ability schema facade `champion_behavior_and_ability_defaults_schema.rs` with explicit sub-leaves under `champion_behavior_and_ability_defaults_schema/*` | typed schema structs only |
...
| Simulation/search/config parse ownership | `data/simulation_search_configuration_parsing.rs` facade plus explicit parse leaves under `data/simulation_search_configuration_parsing/*` (integrated by `data.rs`) | `data/simulation_search_configuration_parsing.rs` facade + explicit parse owner leaves (`shared_parsing_primitives.rs`, `simulation_config_parsing.rs`, `enemy_config_parsing.rs`, `loadout_selection_parsing.rs`) + build-search parse facade `build_search_config_parsing.rs` with explicit sub-leaves under `build_search_config_parsing/*` | `parse_*`, `apply_search_quality_profile`, `loadout_selection_key` owner APIs |
| Loadout-domain legality/modeling ownership | `data/loadout_domain_modeling.rs` facade plus explicit leaves under `data/loadout_domain_modeling/*` (integrated by `data.rs`) | `data/loadout_domain_modeling.rs` facade + explicit leaves (`loadout_domain_schema.rs`, `modeled_rune_filtering.rs`, `rune_page_validation.rs`, `loadout_selection_generation.rs`) | `build_loadout_domain`, `filter_loadout_domain_to_modeled_runes`, `validate_*`, `random_loadout_selection` owner APIs |
...
| Champion/item/preset data loading and URF legality validation | `data/champion_item_preset_data_loading.rs` facade plus explicit data leaves under `data/champion_item_preset_data_loading/*` (integrated by `data.rs`) | `data/champion_item_preset_data_loading.rs` facade + explicit owner leaves (`champion_base_loading.rs`, `urf_mode_loading.rs`, `enemy_preset_loading.rs`) + item-pool facade `item_pool_loading.rs` with explicit sub-leaves under `item_pool_loading/*` | champion/item/preset data loader owner APIs (`load_*`, `default_item_pool`, `validate_enemy_urf_presets`) |
| Reporting markdown run-report rendering ownership | `reporting/controlled_champion_report_markdown_writer.rs` plus explicit section leaves under `reporting/controlled_champion_report_markdown_writer/*` and `reporting/controlled_champion_report_markdown_writer/loadout_and_build_sections/*` (integrated by `reporting.rs`) | `reporting/controlled_champion_report_markdown_writer.rs` facade + explicit section owner leaves (`header_and_objective_sections.rs`, `search_diagnostics_section.rs`, `loadout_and_build_sections.rs`, `loadout_and_build_sections/build_ranking_sections.rs`, `loadout_and_build_sections/enemy_profile_sections.rs`, `loadout_and_build_sections/loadout_profile_sections.rs`) | `write_controlled_champion_report_markdown` facade API plus section-owner `append_*` projection APIs |
...
| Root simulation contract type ownership (runtime/search/reporting contracts and CLI/options contracts) | `main.rs` and crate-root compatibility imports (migrated) | `simulation_contracts.rs` facade + explicit owner leaves (`runtime_actor_contracts.rs`, `search_reporting_contracts.rs`, `entrypoint_cli_contracts.rs`) | typed contracts only; `main.rs` orchestrates via contract exports |
...
- Fixed-loadout rune-sweep read-only aggregation and report projection channels should route through explicit owner leaves (`scenario_runner/rune_sweep_runner/result_aggregation.rs`, `scenario_runner/rune_sweep_runner/report_writing.rs`) rather than expanding `scenario_runner/rune_sweep_runner.rs`.
- Controlled champion scenario orchestration entrypoint logic should route through execution owner modules (`scenario_runner/controlled_champion_scenario_runner.rs` facade + `scenario_runner/controlled_champion_scenario_runner/controlled_champion_scenario_execution.rs` leaf + explicit execution sub-leaves under `controlled_champion_scenario_execution/*`) rather than facade-local execution blocks.
...
- Controlled champion ranked-build analysis/diagnostics and build-order analysis should route through scenario owner modules (`scenario_runner/controlled_champion_result_build_analysis.rs` facade + explicit projection/analysis sub-leaves under `controlled_champion_result_build_analysis/*`) rather than accumulating in `controlled_champion_result_reporting.rs`.
...
- Runtime on-hit and ability bonus-damage resolution (including rune-trigger execution and stack-window progression) should route through `scripts/runtime/loadout_runtime/combat_bonus_resolution.rs` and explicit combat-bonus owner leaves under `scripts/runtime/loadout_runtime/combat_bonus_resolution/*` rather than expanding dense combat bonus logic back into `scripts/runtime/loadout_runtime.rs`.
- Runtime mutation-effect channels (`on_outgoing_damage_heal`, `on_enemy_kill_heal`, `trigger_immobilize_rune_damage`) should route through `scripts/runtime/loadout_runtime/runtime_effect_mutations.rs` rather than scattered mutation logic in `scripts/runtime/loadout_runtime.rs`.
- Runtime state initialization/reset channels should route through `scripts/runtime/loadout_runtime/runtime_state_initialization.rs` rather than inline flag/cooldown map mutations in `scripts/runtime/loadout_runtime.rs`.
- Runtime read-only projection helpers (attack speed, incoming multipliers, movement speed, regeneration) should route through `scripts/runtime/loadout_runtime/runtime_stat_projections.rs` rather than mixed projection/mutation logic in `scripts/runtime/loadout_runtime.rs`.
- Runtime cooldown/stack description projection helpers should route through `scripts/runtime/loadout_runtime/runtime_state_reporting.rs` rather than expanding read-only reporting composition in `scripts/runtime/loadout_runtime.rs`.
...
- Simulator/default schema-type declarations should route through explicit defaults schema modules (`defaults/simulator_defaults_schema_types.rs` facade + `defaults/simulator_defaults_schema_types/*` leaves) rather than expanding facade struct blocks.
...
- Simulation/search/config parse ownership should route through `data/simulation_search_configuration_parsing.rs` facade plus explicit parse owner leaves (`data/simulation_search_configuration_parsing/*`) rather than mixed parser clusters in one leaf file.
- Loadout-domain legality/modeling ownership should route through `data/loadout_domain_modeling.rs` facade plus explicit owner leaves under `data/loadout_domain_modeling/*` rather than facade-local loadout helper clusters.
...
- Champion/item/preset data loading and URF legality validation channels should route through `data/champion_item_preset_data_loading.rs` facade plus explicit owner leaves (`data/champion_item_preset_data_loading/*`) rather than accumulating mixed loading/validation logic in one large leaf file.
- Reporting markdown run-report rendering should route through explicit reporting writer modules (`reporting/controlled_champion_report_markdown_writer.rs` facade + `reporting/controlled_champion_report_markdown_writer/*` section owners) rather than facade-local report assembly clusters.
...
- Current status (`2026-02-24`): `0` `mod.rs` files remain under `src/`.
...
- If a temporary `mod.rs` is introduced for staged compatibility, track removal under `ARCH-060`/`ARCH-070` and remove it as soon as call sites are stable.
