...
| Scenario fixed-loadout and rune-sweep execution entrypoints | `scenario_runner/fixed_loadout_runner.rs`, `scenario_runner/rune_sweep_runner.rs`, and explicit rune-sweep leaves under `scenario_runner/rune_sweep_runner/*` (integrated by `scenario_runner.rs`) | `scenario_runner/fixed_loadout_runner.rs` + `scenario_runner/rune_sweep_runner.rs` facade + explicit rune-sweep owner leaves (`result_aggregation.rs`, `report_writing.rs`) | fixed-loadout execution owner APIs plus rune-sweep owner APIs (`average_*`, `write_rune_sweep_reports`) |
| Scenario controlled-champion execution entrypoint | `scenario_runner/controlled_champion_scenario_runner.rs` facade plus explicit execution leaf `scenario_runner/controlled_champion_scenario_runner/controlled_champion_scenario_execution.rs` (integrated by `scenario_runner.rs`) | `scenario_runner/controlled_champion_scenario_runner.rs` facade + `scenario_runner/controlled_champion_scenario_runner/controlled_champion_scenario_execution.rs` execution owner leaf | `run_controlled_champion_scenario_impl` owner API |
