...
- Champion controller harness runtime ingress now routes deterministic actor-id keyed command handling through `Simulation/src/champion_control_harness/*` + `Simulation/src/engine/controlled_champion_controller_channels.rs` with sequence-ordered request execution, fixed tick-delay command application, command-owned movement stepping, and partial actor-symmetric opponent move/stop command channels.
